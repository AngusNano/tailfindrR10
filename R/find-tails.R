...
find_tails <- function(fast5_dir,
    ...
    version <- utils::packageDescription("tailfindrR10")$Version
    cat(cli::rule(left=''), '\n', sep = "")
    cat(cli::rule(left=paste("Started tailfindrR10 ", '(version ', version, ')', sep='')), '\n', sep = "")
    cat(cli::rule(left=''), '\n',  sep = "")

    # display the user-specified parameters
    cat(paste(cli::symbol$menu, ' You have configured tailfindrR10 as following:', '\n', sep=''))
